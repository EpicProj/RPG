        ...
        public override void GenerateOnly()
        {
            ...
            if (voxelObject.materialMode == VoxelChunksObject.MaterialMode.Combine)
            {
                if (explosionObject.materials == null)
                    explosionObject.materials = new List<UnityEngine.Material>();
                ...
                for (int i = 0; i < voxelBase.materialData.Count; i++)
                {
                    if (explosionObject.materials[i] == null)
                    {
                        if (!voxelBase.materialData[i].transparent)
                            explosionObject.materials[i] = new UnityEngine.Material(Shader.Find("Voxel Importer/Explosion/VoxelExplosion-Opaque"));
                        else
                            explosionObject.materials[i] = new UnityEngine.Material(Shader.Find("Voxel Importer/Explosion/VoxelExplosion-Transparent"));
                    }
                    else
                    {
                        if (!voxelBase.materialData[i].transparent)
            ...
            else if (voxelObject.materialMode == VoxelChunksObject.MaterialMode.Individual)
            {
                ...
                for (int chunkIndex = 0; chunkIndex < explosionObject.chunksExplosion.Length; chunkIndex++)
                {
                    ...
                    if (explosionChunk.materials == null)
                        explosionChunk.materials = new List<UnityEngine.Material>();
                    ...
                    for (int i = 0; i < voxelBase.materialData.Count; i++)
                    {
                        ...
                        {
                            if (!voxelBase.materialData[i].transparent)
                                explosionChunk.materials[i] = new UnityEngine.Material(Shader.Find("Voxel Importer/Explosion/VoxelExplosion-Opaque"));
                            else
                                explosionChunk.materials[i] = new UnityEngine.Material(Shader.Find("Voxel Importer/Explosion/VoxelExplosion-Transparent"));
                        }
                        else
                        {
                            if (!voxelBase.materialData[i].transparent)
