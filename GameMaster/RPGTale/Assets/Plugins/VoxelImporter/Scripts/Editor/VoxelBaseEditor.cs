...
namespace VoxelImporter
{
    public abstract class VoxelBaseEditor : Editor
    {
        ...
        protected void InspectorGUI_ImportSettingsImportScale()
        {
            ...
            if (GUILayout.Button("Set", guiStyleDropDown, GUILayout.Width(40), GUILayout.Height(14)))
            {
                ...
                #region Division
                {
                    ...
                    foreach (var value in ScaleTemplate)
                    {
        ...
        protected abstract List<UnityEngine.Material> GetMaterialListMaterials();
                                    ...
                                    if (!string.IsNullOrEmpty(path))
                                    {
                                        ...
                                        else
                                        {
                                            EditorApplication.delayCall += () =>
                                            {
                                                ...
                                                path = FileUtil.GetProjectRelativePath(path);
                                                AssetDatabase.CreateAsset(UnityEngine.Material.Instantiate(materials[index]), path);
                                                materials[index] = AssetDatabase.LoadAssetAtPath<UnityEngine.Material>(path);
                                                Refresh();
                                            };
                                        }
                                    }
