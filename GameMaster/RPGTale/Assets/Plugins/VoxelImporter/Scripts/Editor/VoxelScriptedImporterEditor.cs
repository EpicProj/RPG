                                    ...
                                    try
                                    {
                                        AssetDatabase.StartAssetEditing();
                                        {
                                            foreach (var t in targets)
                                            {
                                                ...
                                                var materials = AssetDatabase.LoadAllAssetsAtPath(importer.assetPath).Where(x => x.GetType() == typeof(UnityEngine.Material));
                                                foreach (var material in materials)
                                                {
                                                    ...
                                                    AssetDatabase.CreateAsset(UnityEngine.Material.Instantiate(material), assetPath);
                                                    {
                                                        var saveMaterial = UnityEditor.AssetDatabase.LoadAssetAtPath<UnityEngine.Material>(assetPath);
                    ...
                    if (targets.Length == 1)    //single selection only
                    {
                        ...
                        for (int index = 0; index < vtarget.materialNames.Length; index++)
                        {
                            ...
                            for (int i = 0, count = remappedMaterialsProp.arraySize; i < count; i++)
                            {
                                ...
                                {
                                    materialProp = prop;
                                    material = prop.objectReferenceValue as UnityEngine.Material;
                                    propertyIdx = i;
                                    break;
                                }
                            }
                            ...
                            {
                                EditorGUI.BeginChangeCheck();
                                material = EditorGUILayout.ObjectField(vtarget.materialNames[index], material, typeof(UnityEngine.Material), false) as UnityEngine.Material;
                                if (EditorGUI.EndChangeCheck())
                                {
                                    if (material != null)
                                    {
