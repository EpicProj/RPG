...
namespace VoxelImporter
{
    ...
    public class VoxelChunksObjectEditor : VoxelBaseEditor
    {
        ...
        protected override List<UnityEngine.Material> GetMaterialListMaterials()
        {
            ...
            if (objectTarget.materialMode == VoxelChunksObject.MaterialMode.Combine)
            {
                ...
                if (objectTarget.materials != null)
                {
                    for (int index = 0; index < objectTarget.materials.Count; index++)
                    {
                        ...
                        AssetDatabase.CreateAsset(UnityEngine.Material.Instantiate(objectTarget.materials[index]), path);
                        objectTarget.materials[index] = AssetDatabase.LoadAssetAtPath<UnityEngine.Material>(path);
                    }
                }
                #endregion
                    ...
                    for (int i = 0; i < objectTarget.chunks.Length; i++)
                    {
                        ...
                        if (objectTarget.chunks[i].materials != null)
                        {
                            for (int index = 0; index < objectTarget.chunks[i].materials.Count; index++)
                            {
                                ...
                                path = FileUtil.GetProjectRelativePath(path);
                                path = AssetDatabase.GenerateUniqueAssetPath(path);
                                AssetDatabase.CreateAsset(UnityEngine.Material.Instantiate(objectTarget.chunks[i].materials[index]), path);
                                objectTarget.chunks[i].materials[index] = AssetDatabase.LoadAssetAtPath<UnityEngine.Material>(path);
                            }
                        }
                        #endregion
